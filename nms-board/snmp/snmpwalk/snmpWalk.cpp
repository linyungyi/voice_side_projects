...
#define BULK_MAX 20     // Number of request in the getbulk
#define DEFAULT_OID "1" // Default is begining of the Mib
...
int  main( int argc, char **argv)  {
   ...
   if ( argc < 2) {
      printf("Usage:\n");
      printf("snmpWalk Address | DNSName [CNmsSnmpOid] [options]\n");
      ...
      printf("         -lN , maximum number of loops (default:0=>infinite)\n");
      return 1;
   }
   ...
   unsigned int nbloops = 0;                                        // infinite by default
   ...
   for(int x=1;x<argc;x++) {                           // parse for version
      ...
      if ( strstr( argv[x], "-l")!=0) {                 // parse for nb loop
         ptr = argv[x]; ptr++; ptr++; 
         nbloops = atoi( ptr);
      }
      if ( strstr( argv[x],"-s")!= 0)                   // Silence mode activated
         silent_mode = 1;
   ...
   printf("SNMP++ Walk to %s SNMPV%d Retries=%d", argv[1], (version+1), retries);
   printf(" Port=%d Timeout=%d ms Community=%s Nbloops=%d\n",target.get_port(), timeout, community.get_printable(), nbloops);
   while (( status = snmp.get_bulk( pdu,target,0,BULK_MAX))== SNMP_CLASS_SUCCESS) {
      ...
      for ( int z=0;z<pdu.get_vb_count(); z++) {
         ...
         if ( vb.get_syntax() != sNMP_SYNTAX_ENDOFMIBVIEW) {
            ...
            if(nbloops==1) 
            {
                printf("Nbloop reached...End of Walk\n");
                printf("Total # of Requests = %d\n", requests);
                printf("Total # of Objects  = %d\n", objects);
                return 1;
            }
            if(nbloops>0) nbloops--;
         }
         else {
            printf("End of MIB Reached\n");
            printf("Total # of Requests = %d\n", requests);
            printf("Total # of Objects  = %d\n", objects);
            return 1;
         }
   ...
   if(status != SNMP_CLASS_SUCCESS)
   {
      ...
      if(nErrorIndex>0)
        ...
        pdu.get_vb( vb,0);
      printf("SNMP++ Walk Error [error_index=%d related with %s]\n",nErrorIndex, vb.get_printable_oid());
