...
int main( int argc, char **argv)  {
   ...
   if ( argc < 2) {
	  printf("Usage:\n");
	  printf("snmpNext Address | DNSName [CNmsSnmpOid] [options]\n");
   ...
   target.set_readcommunity( community);  // set the read community to use
   target.set_port( port);				   // set the SNMP port
 
   //-------[ issue the request, blocked mode ]-----------------------------
   printf("SNMP++ GetNext to %s SNMPV%d Retries=%d", argv[1], (version+1), retries);
   ...
   if (( status = snmp.get_next( pdu,target))== SNMP_CLASS_SUCCESS) 
   {
	  ...
	  if(nErrorIndex>0)
		...
		pdu.get_vb( vb,0);
	  printf("SNMP++ GetNext Error [error_index=%d related with %s]\n",nErrorIndex, (LPCSTR) vb.get_printable_oid());
