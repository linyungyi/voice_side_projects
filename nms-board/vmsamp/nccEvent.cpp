...
DWORD enableAudByChannel( int nGw, MSPHD hd );
...
{ 
    ...
    if( (mmParm.Audio_RTP_Format != NO_AUDIO) && 
      ( (AudioCtx[pCtx->dCXIndex].pr_mode == PR_MODE_ACTIVE1) || (AudioCtx[pCtx->dCXIndex].pr_mode == PR_MODE_ACTIVE2) ) )
        	cmdStopAudio( pCtx->dCXIndex );				

    if( (mmParm.Video_Format != NO_VIDEO) && 
      ( (VideoCtx[pCtx->dCXIndex].pr_mode == PR_MODE_ACTIVE1) || (VideoCtx[pCtx->dCXIndex].pr_mode == PR_MODE_ACTIVE2) ) )
...
{ 
	...
	enableAudByChannel( pCtx->dCXIndex, GwConfig[pCtx->dCXIndex].AudByChan.hd );
...
DWORD disableVideoChannel( int nGw, MSPHD hd )
{
	...
	if( !GwConfig[nGw].VidChan.bEnabled ) {
	...
	if (ret != SUCCESS) {
    	...
    	if (ret != 0) {
			...
			GwConfig[nGw].VidChan.bEnabled = FALSE;
	}
...
DWORD disableAudByChannel( int nGw, MSPHD hd )
{
	...
	if( !GwConfig[nGw].AudByChan.bEnabled ) {
	...
	if (ret != SUCCESS) {
    	...
    	if (ret != 0) {
			...
			GwConfig[nGw].AudByChan.bEnabled = FALSE;
	}
...
DWORD enableAudByChannel( int nGw, MSPHD hd )
{
    ...
    if (ret != SUCCESS) {
    	...
    	if (ret != 0) {
			...
			GwConfig[nGw].AudByChan.bEnabled = TRUE;
	}
...
DWORD enableVideoChannel( int nGw, MSPHD hd )
{
    ...
    if (ret != SUCCESS) {
    	...
    	if (ret != 0) {
			...
			GwConfig[nGw].VidChan.bEnabled = TRUE;
	}
