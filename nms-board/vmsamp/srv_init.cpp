...
void SelectEncoders()
{
	...
	switch (mmParm.Audio_RTP_Format) 
	{
		case G723_5:
			...
			mmParm.FusChanType = G723FullDuplex;
...
void InitMsppConfiguration()
{
	...
	for (i=0; i<mmParm.nPorts; i++)
	{
        VideoCtx[i].pr_mode = PR_MODE_INIT;
		...
		if(mmParm.Video_Format != NO_VIDEO)
		{
			...
			VideoCtx[i].rtpEp.Addr.EP.RtpRtcp.nSrcPort 		= mmParm.srcPort_video + i*2;
			strcpy(VideoCtx[i].rtpEp.Addr.EP.RtpRtcp.DestIpAddress, mmParm.destIPAddr_video);
			VideoCtx[i].rtpEp.Addr.EP.RtpRtcp.nDestPort 	= mmParm.destPort_video + i*2;
        ...
        AudioCtx[i].pr_mode = PR_MODE_INIT;
        if( mmParm.audioMode1 != NONE )		
			...
			AudioCtx[i].rtpEp.Addr.EP.RtpRtcp.nSrcPort 		= mmParm.srcPort_audio + i*2;
			strcpy(AudioCtx[i].rtpEp.Addr.EP.RtpRtcp.DestIpAddress, mmParm.destIPAddr_audio);
			AudioCtx[i].rtpEp.Addr.EP.RtpRtcp.nDestPort 	= mmParm.destPort_audio + i*2;
            ...
            pVoiceParm->EncoderParms.Rate                     = H2NMS_WORD(mmParm.FusEncodeRate);
