/*****************************************************************************
  ...
  PURPOSE:
      ...
      To test functionality of the Event Tracking (EVT) Service,
      which uses the Runtime Control features (RTC),
      with the Software Timer (TIK) service as the event provider
      and the Event Tracking (EVT) Service as the event consumer.
...
typedef struct _DEMO_OBJ
{
   ...
   char             *LogFileName; /* event log file name                 */
}  DEMO_OBJ;
...
#define  TIKTST_DEFAULT_DEMO_MODE   3
/******************************************************************************
...
static void tikTstUsage( void )
{
   ...
   printf( "Usage: tiktst [opts]\n"
           "  where opts are:\n"
           "  -l (event log file name, e.g., \"evt_0819.log\")      \n"
           "     use standard output file is the -l option is absent\n"
...
static void tikTstParseOption( int argc, char **argv, DEMO_OBJ *Demo )
{
   ...
   */
   while ( (i = getopt( argc, argv, "f:l:m:n:tHh?")) != -1 )
   {
      switch (i)
      {
         case 'l':
            Demo->LogFileName = optarg;
            break;
         case 'n':
            if ( Demo->Start == NULL )
            {
...
static void tikTstRunDemoMode1( DEMO_OBJ *Demo )
{
   ...
   */
   for ( Ctr = 0; Ctr < TIKSVR_MAX_CHANNEL; Ctr++ )
   {
      ...
      Ret = ctaOpenServices( CtaHd[Ctr], tikOpenSvcLst, 2 );
      if ( Ret != SUCCESS )
      {
      ...
      /* Set the event log file name for the entire application. */
      if (Ctr == 0)
      {
         evtSetLogFile( CtaHd[0], Demo->LogFileName );
      }

      /* Start logging the events. */
      evtLogStart( CtaHd[Ctr] );

      /* Add all TIK event IDs to the tracking list. */
      evtAddEventAll( CtaHd[Ctr], TIK_SVCID );
   ...
   */
   for ( Ctr = 0; Ctr < TIKSVR_MAX_CHANNEL; Ctr++ )
   {
      /* Remove all TIK event IDs from the tracking list. */
      evtRemoveEventAll( CtaHd[Ctr], TIK_SVCID );

      /* Stop logging the events. */
      evtLogStop( CtaHd[Ctr] );

      Ret = ctaCloseServices( CtaHd[Ctr], tikCloseSvcLst, 2 );
      if ( Ret != SUCCESS )
      {
...
static void tikTstRunDemoMode2( DEMO_OBJ *Demo )
{
   ...
   /* Open logical channel 0 on context.          */
   {
      ...
      Ret = ctaOpenServices( FstCtaHd, tikOpenSvcLst, 2 );
      if ( Ret != SUCCESS )
      {
      ...
      /* Set the event log file name for the entire application. */
      evtSetLogFile( FstCtaHd, Demo->LogFileName );

      /* Start logging the events. */
      evtLogStart( FstCtaHd );

      /* Add a specific event ID and event value pair to the tracking list. */
      evtAddEventValue( FstCtaHd, TIKEVN_TIMER_DONE, CTA_REASON_FINISHED );
   ...
   /* Open logical channel 0 on context.          */
   {
      ...
      Ret = ctaOpenServices( SndCtaHd, tikOpenSvcLst, 2 );
      if ( Ret != SUCCESS )
      {
      ...
      /* Start logging the events. */
      evtLogStart( SndCtaHd );

      /* Add a specific event ID to the tracking list. */
      evtAddEvent( SndCtaHd, TIKEVN_TIMER_TICK );
   ...
   */
   ...
   /* Remove a specific event ID and event value pair from the tracking list. */
   evtRemoveEventValue( FstCtaHd, TIKEVN_TIMER_DONE, CTA_REASON_FINISHED );

   /* Remove a specific event ID from the tracking list. */
   evtRemoveEvent     ( SndCtaHd, TIKEVN_TIMER_TICK );
   ...
   *   Close service and destroy context.
   */
   Ret = ctaCloseServices( FstCtaHd, tikCloseSvcLst, 2 );
   if ( Ret != SUCCESS )
   {
   ...
   Ret = ctaCloseServices( SndCtaHd, tikCloseSvcLst, 2 );
   if ( Ret != SUCCESS )
   {
...
static void tikTstRunDemoMode3( DEMO_OBJ *Demo )
{
   ...
   /* Open logical channel 0 on context.          */
   {
      ...
      /* Add all TIK event IDs to the tracking list. */
      evtAddEventAll( FstCtaHd, TIK_SVCID );
   ...
   */
   ...
   /* Remove all TIK event IDs from the tracking list. */
   evtRemoveEventAll  ( FstCtaHd, TIK_SVCID );
...
{
   /* Demo object of current running evttest.exe */
   DEMO_OBJ   Demo  = { 0 };

   /* Natural Access variables. */
   CTA_INIT_PARMS      initparms =       { 0 };
   CTA_ERROR_HANDLER   oldErrorHandler = NULL;
   CTA_SERVICE_NAME    serviceNames[] =  { { "EVT", "EVTMGR"}, { "TIK", "TIKMGR" } };

   /* Utility variable. */
   DWORD      Ret;
   unsigned   DaemonRunning = 1;

   printf( "EVT Service Demo V.%s (%s)\n\n", VERSION_NUM, VERSION_DATE );
   ...
   *   run mode 2, 3.
   */
   Ret = ctaInitialize(serviceNames, 2, &initparms);
   if (Ret != SUCCESS)
   {
