...
int            VmTraceMask = 0x47D;  // No T_H245_DET - 1111101
...
int ModifySrvParms( char *FileName )
{
    ...
    {     /* Read a line from config file; break on EOF or error. */
        ...
        case AUTO_PLAY_LIST:
            ...
            break;
                ...
                continue;
                        ...
                        mmParm.bAutoPlayRec = value.numval;
                        mmPrintAll("AutoPlayRec = %d\n", mmParm.bAutoPlayRec);
                        break;
        ...
        case AUTO_PLAY_LIST_FILE_NAME:
            ...
            break;                        
        case THREEGP_FILE_NAME_PLAY:
            if ( getargs(p,&value, STRINGVALUE) != SUCCESS )
            {
                continue;
            }
            strcpy(mmParm.threegpFileName_play, value.strval);
            mmPrintAll("3GP Play File Name = %s\n", mmParm.threegpFileName_play);
            break;                            
        case THREEGP_FILE_NAME:
            ...
            break;
        case PARTIAL_BUFFERS:
            if ( getargs(p,&value, NUMVALUE) != SUCCESS )
            {
                continue;
            }
            mmParm.bPartialBuffers = value.numval;
            mmPrintAll("Partial Buffers = %d\n", mmParm.bPartialBuffers);
            break;
        case BOARD_TYPE:
            if ( getargs(p,&value, NUMVALUE) != SUCCESS )
            {
            ...
            if ((value.numval != 6000) && (value.numval != 6500) && (value.numval != 6565))
            {
                printf("Invalid nBoardType option (only 6000, 6500, and 6565 are valid)\n");
                continue;
            }
...
int get_keyword (char **bufp, argval *pval)
{
    ...
    static struct {
        ...
        {"AutoPlay",                    AUTO_PLAY                   },                
        {"AutoPlayRec",                 AUTO_PLAY_REC               },
        {"3gpFileName_play",            THREEGP_FILE_NAME_PLAY      },    
        {"3gpFileName",                 THREEGP_FILE_NAME           },
        {"PartialBuffers",              PARTIAL_BUFFERS             },
