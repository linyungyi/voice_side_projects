...
extern int     getVideoChannelStatus(int chanId);
    ...
    while (display_menu)
    {
        ...
        switch (selection)
	    {
            ...
            case 'f':  // H245 User Indication - alphanumeric
                ...
                // Send UII
				rc = h324UserIndication( (MSPHD)GwConfig[0].MuxEp.hd, &l_UII );
				if ( rc == SUCCESS ) 
                {
            ...
            case 'o':
                ...
                rc = h324VendorIDIndication((MSPHD)GwConfig[0].MuxEp.hd, l_pVID);
                if (rc == SUCCESS) 
                {
            ...
            case 'p':
                ...
                rc = h324VideoTemporalSpatialTradeoffIndication((MSPHD)GwConfig[0].MuxEp.hd, tradeoff);
                if (rc == SUCCESS) 
                {
...
DWORD configurePlayWithXc( int nGw )
{
    ...
    // dynamic TRC channel
    if(getVideoChannelStatus(pCfg->VideoXC.xcChanId)==TRC_CHAN_ACTIVE){ 
