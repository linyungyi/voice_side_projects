...
#ifdef LINUX
/* The same as scanf but strips leading '\n' symbols */
void read_parameter(char *format, ... )
{
    va_list other_args;
	char buf[128];
	int i;
	...
	/* Zero the buffer */
	memset( (void *)buf, 128, '\0' );
	
	va_start(other_args, format);			
	
	/* Skip all '\n' symbols */
	i = 0;
	while( (buf[i++] = getc(stdin)) == '\n' );
	
	/* Read the rest of the string */
	fgets( &buf[i], 64, stdin );

	vsscanf( &buf[i-1], format, other_args );	
